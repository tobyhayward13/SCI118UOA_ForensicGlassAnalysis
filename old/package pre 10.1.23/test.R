...
# undebug(partition_fast)

part = partition(ri)
part.c = partition_C(ri)
part.fast = partition_fast(ri)
...
# Speed Testing
ri <- generate_indices(30, .sd_multi = 10)
ri

time_data = data.frame(
  t1 = system.time(partition(ri))[3],
  t2 = system.time(partition_C(ri))[3],
  t3 = system.time(partition_fast(ri))[3]
)
